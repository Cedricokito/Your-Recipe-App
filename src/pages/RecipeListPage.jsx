    ...
    inputValue === ""
      ...
      : data.hits.filter(
          (recipe) =>
            ...
            isVeganOrVegetarian(recipe.recipe.healthLabels)
              .toLowerCase()
              .includes(inputValue)
        );
      ...
      >
        {filteredRecipes.map((recipe, index) => (
          <Box
            ...
            cursor='pointer'
            onClick={() =>
              navigate(`/recipepage?recipe=${recipe.recipe.label}`)
            }
          >
            <Image
              h='15vh'
            ...
            <Box p={4}>
              ...
              <Heading fontSize='1.1rem' textAlign='center' mt={2}>
                {recipe.recipe.label}
              </Heading>
              <Text
                bg='rgb(227, 157, 227)'
              ...
              {recipe.recipe.dishType &&
                recipe.recipe.dishType.map((type, index) => (
                  <Text textAlign='center' key={index} mt={2}>
                    Dish: {type}
                  </Text>
                ))}
              {recipe.recipe.cautions && recipe.recipe.cautions.length > 0 && (
                <Heading mt={2} fontSize='1rem' textAlign='center'>
                  Cautions
                </Heading>
              )}
              <Flex flexWrap='wrap' mt={2} justifyContent='center'>
